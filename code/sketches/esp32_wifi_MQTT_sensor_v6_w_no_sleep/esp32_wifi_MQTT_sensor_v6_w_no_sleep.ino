...
// ===== DEBUG =====
#define DEBUG_ENABLED  true      // <<< set false to silence debug output
#define HEARTBEAT_MS   10000UL   // periodic status print when debug is on
...
// ===== CONFIG =====
#define BIN_SECONDS      10   // 300 in production?
#define NUM_BINS         3    // 12 in production?
...
// ISR debug state
volatile uint32_t isrPulseCountSinceLastReport = 0;
volatile uint64_t lastPulseUs = 0;
volatile bool     pulseSeenSinceLastHeartbeat = false;
...
void printSystemStatus(const char* tag) {
    if (!DEBUG_ENABLED) return;

    uint64_t nowUs = esp_timer_get_time();
    uint64_t secsToBoundary = (nextBin_us > nowUs) ? ((nextBin_us - nowUs) / 1000000ULL) : 0;

    noInterrupts();
    uint32_t localPulseBurst = isrPulseCountSinceLastReport;
    uint64_t localLastPulseUs = lastPulseUs;
    bool localPulseSeen = pulseSeenSinceLastHeartbeat;
    isrPulseCountSinceLastReport = 0;
    pulseSeenSinceLastHeartbeat = false;
    uint32_t localTotal = totalPulses;
    uint32_t localCounts[NUM_BINS];
    for (int i = 0; i < NUM_BINS; i++) localCounts[i] = counts[i];
    interrupts();
    ...
    DBG_PRINTF(
        "[HEARTBEAT] %s | uptime=%lu ms | freeHeap=%u | wifi=%d | mqtt=%d | RSSI=%d | currentBin=%u | nextBinIn=%llu s | total=%u | recentISR=%u | lastPulseAge=%llu ms | pin=%d\n",
        tag,
        millis(),
        ESP.getFreeHeap(),
        WiFi.status(),
        ...
        localTotal,
        localPulseBurst,
        (localLastPulseUs == 0 ? 0ULL : (nowUs - localLastPulseUs) / 1000ULL),
        digitalRead(SENSOR_PIN)
    );

    DBG_PRINT("[HEARTBEAT] binCounts=[");
    for (int i = 0; i < NUM_BINS; i++) {
        DBG_PRINT(localCounts[i]);
        if (i < NUM_BINS - 1) DBG_PRINT(",");
    }
    ...
    if (localPulseSeen) {
        DBG_PRINTLN("[HEARTBEAT] Pulse activity detected since last heartbeat.");
    } else {
        DBG_PRINTLN("[HEARTBEAT] No pulse activity detected since last heartbeat.");
    }
}
...
void IRAM_ATTR pulseISR() {
    ...
    lastPulseUs = now;
    isrPulseCountSinceLastReport++;
    pulseSeenSinceLastHeartbeat = true;
}
...
void publishBins() {
    ...
    // Keep original behavior of always printing payload
    Serial.println(json);
...
void handleBinsAndUpload() {
    while (true) {
        ...
        if (currentBin >= NUM_BINS) {
            ...
            DBG_PRINTLN("[CPU] Frequency set to 80 MHz for Wi-Fi/MQTT");
            ...
            DBG_PRINTLN("[CPU] Frequency restored to 40 MHz");
...
void setup() {
    ...
    setCpuFrequencyMhz(40);
    DBG_PRINTF("[CPU] Baseline frequency set to %u MHz\n", getCpuFrequencyMhz());

    esp_bt_controller_disable();
    btStop();
    DBG_PRINTLN("[BT] Bluetooth disabled");
...
void loop() {
    ...
    if (DEBUG_ENABLED && (millis() - lastHeartbeatMs >= HEARTBEAT_MS)) {
        lastHeartbeatMs = millis();
        printSystemStatus("loop");
    }
